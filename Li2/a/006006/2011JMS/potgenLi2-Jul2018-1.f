c***********************************************************************
      SUBROUTINE POTGEN(ISTATE,IMN1,IMN2,NPP,VLIM,XO,RM2,VV)
c***********************************************************************
c     SUBROUTINE POTGEN(LNPT,NPP,IAN1,IAN2,IMN1,IMN2,VLIM,XO,RM2,VV,
c    1                                                        NCN,CNN)
c***********************************************************************
c                       Version of   27 July 2018
...
ccc Specify parameter values for the two Li2 states of interest
     ...
     1  NLRA/16,3,16,9/, NU1A/2,0,4,3/,QPARA/3,3,3,2/ 
      DATA DSCMA/8516.708d0,333.778d0,9353.1795d0,7093.4926d0/,
     1  REQA/2.6729874d0,4.17001d0,3.1079288d0,3.065436d0/,
     2  RrefA/4.07d0,8.0d0,4.4d0,4.8d0/, 
c** specify the 4 C_m long-range coefficients for the 4 states
     4  CMMA/6.7190d6,1.12634d8,2.78694d9,0.d0, 6.7190d6,1.12634d8,
...
c** now specify 20 exponent parameters \beta_i for the 4 states
     ...
     a   -1.5D+01,3*0.D0,
     b   -5.16086D-01,-9.78D-02,1.136D-01,-2.49D-02, 16*0.d0,
...
c-----------------------------------------------------------------------
c             WRITE(6,630) 1,MASS1,MN1R,NAME1,IMN1,NAME1,
c    1        1,U1INF,PAD,PAD,PAD,PAD,PAD,PAD,NU1,QAD,QAD,QAD,QAD,QAD,
c    2                                         NU1+1,(U1(I),I= 0,NU1)
...
c-----------------------------------------------------------------------
c             WRITE(6,630) 2,MASS2,MN2R,NAME2,IMN2,NAME2,
c    1        1,U2INF,PAD,PAD,PAD,PAD,PAD,PAD,NU2,QAD,QAD,QAD,QAD,QAD,
c    2                                         NU2+1,(U2(I),I= 0,NU2)
...
c             WRITE(6,602) NCN,PPAR,QPAR,DSCM,REQ
c... use THEOCHEM/Huang form:  \beta(yp)= Binf*yp + [1-yp]*{power series in yq}
c             WRITE(6,607) PPAR,PPAR,QPAR,NSR,NLR,IORD+1,
c    1                                       (PARM(J),J= 1,IORD+1)
              IF(Rref.GT.0) THEN
c                 WRITE(6,613) Rref
                ELSE
                  WRITE(6,615) REQ
              ...
              IF(rhoAB.GT.0.d0) THEN
                  ...
                  IF(IDSTT.GT.0) THEN
                      PVSR= 0.5d0*IVSR
c                     WRITE(6,664) rhoAB,PVSR,bDS(IVSR),cDS(IVSR),PVSR
                    ELSE
                      LVSR= IVSR/2
...
c                 WRITE(6,668)
                ENDIF
c             WRITE(6,617) BINF,MMLR(1),CMM(1),MMLR(1)
              IF(NCMM.GT.1) THEN
                  MM1= 2
                  IF(MMLR(2).LE.0) THEN
                      MM1= 3
c                     IF(MMLR(2).EQ.0)
c    1                         WRITE(6,623) MMLR(2),CMM(2),MMLR(2)
c                     IF(MMLR(2).LT.0) WRITE(6,625) MMLR(2),CMM(2),0
                      ENDIF
                  DO  I= MM1,NCMM
c                     IF(MMLR(I).LE.9) WRITE(6,619) MMLR(I),CMM(I)
c    1                                                    ,MMLR(I)
c                     IF(MMLR(I).GT.9) WRITE(6,621) MMLR(I),CMM(I)
c    1                                                    ,MMLR(I)
                      ENDDO
                  ENDIF
              ENDIF
